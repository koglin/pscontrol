...
import time
import pyca
from pscontrol import Device, Message
...
class IMS(Device):
    """Epics IMS Motor Class updated from pyepics as an example for use with lcls pyca 
...
#  This is a simple way:
        ...
        if 1 == stat:
            if wait:  # ... and finished OK
                ...
                if 1 == self.get('LVIO') or confirm_move:
                    pyca.pend_io(timeout=1.e-2)
                moving = False
                if confirm_move:
                    t0 = time.time()
                    while self.get('MOVN')==0:
                        pyca.pend_io(timeout=1.e-3)
    ...
    def set_callback(self, attr='VAL', callback=None, kws=None):
        ...
        index = self.PV(attr).add_monitor_callback(callback=callback, **kw_args)
...
#        return steplist
    ...
    def show_log(self, **kwargs):
        ...
        for attr in self._log_attrs:
            message.add(self.get(attr, as_string=True))
        message.add("--------------------------------------")

    def show_cfg(self, **kwargs):
        """Show configuration file.
        """
        message = kwargs.get('message', Message())
        message.add( "--------------------------------------")
        message.add(self.get('CFG_FILE', as_string=True))
        message.add("--------------------------------------")
