...
class Device(smartDevice.Device):
    ...
    _nonpvs = ('_prefix', '_pvs', '_delim', '_init', 
               '_expand_records', '_info_attrs', '_log_attrs', 
    ...
    def show_info(self, **kwargs):
        ...
        if self._fields:
            self.show_attrs(self._info_attrs, message=message, **kwargs)
        else:
            self.show_records(message=message, **kwargs)

    def show_all(self, **kwargs):
        """Show all pv attributes except for log messages. 
        """
        self.show_attrs(**kwargs)

    def show_attrs(self, *args, **kwargs):
        """Show pv attributes.
        """
        if len(args) == 1:
        ...
        # Need to make sure connected
        needsleep = False
        for attr in attrs:
            if attr not in self._pvs:
                value = self.get(attr, as_string=True, timeout=0.1)
                #if not value:
                #    value = self.get(attr, as_string=True, timeout=1.)
                needsleep = True
      
        if needsleep is True:
            time.sleep(0.05)
        ...
        message.add('#'+'-'*80)
        message.add('# {:14} -- {:50}'.format(self.name,self.DESC))
        message.add('# {:7} {:22} {:22} {:30}'.format("Attr","Alias","Value","Description"))
        message.add('#'+'-'*80)
        ...
        for attr in attrs:
            ...
            if value is None:
                value = 'Not Connected??'
            message.add('{:9} {:22} {:22} {:30}'.format(attr, alias, value, desc))

    def show_records(self, attrs=None, expand=False, prefix=None, **kwargs):
        ...
        for attr in attrs:
            ...
            if hasattr(rec, 'RTYP') and rec.RTYP:
                if hasattr(rec, 'RBV'):
                ...
                message.add('{:30s} {:>12} {:6} {:30}'.format(alias, value, units, desc))

            if expand:
                rec.show_records(expand=expand, prefix=alias, message=message)
...
class EpicsSets(object):
    ...
    def __init__(self, instrument=None, pvs=None, 
            no_auto_load=False, quiet=False, **kwargs):
    ...
    def load_epicsCameras(self, epics_file=None, epics_dir=None, **kwargs):
        ...
        if not epics_dir and self.instrument:
            epics_dir = '/reg/g/pcds/pyps/config/'+self.instrument+'/'

        if epics_dir:
            print 'instrument: {:}'.format(self.instrument)
            print 'loading epics pvs from', epics_file, ' in', epics_dir
            self._epics_camdict = camviewer_dict(epics_file,epics_dir)
...
class EpicsDeviceSet(object):
    ...
    def __init__(self, name, alias_dict, noInit=True, quiet=False, **kwargs):
        self._name = name
        self._edm_screen = None
        self._edm_path = None
        self._quiet = quiet
...
#    def epics_table(self, show_all=False, *aliases, **kwargs):
    def epics_table(self, *args, **kwargs):
        ...
        for pvtype, pvlist in pv_types.items():
            if pvtype:
                pvs = list(sorted(set(pvlist)))
                ...
                column_format = ' {:>'+str(min(max(12,140/len(pvs)),wmax))+'}'
                for pv in pvs:
...
#                        precision = 0
                    ...
                    # pv class is supposed to have units but does not work.
                    # Teddy will look into this with Mike Browne
                    #if pv.units:
                    #    units = pv.units
                    if hasattr(epics_device, 'EGU'):
                        units = epics_device.EGU
    ...
    def open_edm(self, **kwargs):
        if not self._edm_screen_maker:
            self._create_edm_screen(**kwargs)

        screen_maker.open_edm(self._edm_screen_maker.screen_file)
        
    def _create_edm_screen(self, filename=None, path=None):
        if path:
        ...
        if not filename:
            filename = '{:}.edl'.format(self._name)

        self._edm_screen_maker = screen_maker.EDM_Screen_Maker(filename,
                                    self._aliases.values(),
                                    group_name=self._name,
                                    path=path)
        self._edm_path = path  
        self._edm_screen_maker.create_screen()
