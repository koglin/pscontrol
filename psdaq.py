...
class Daq(object):
    ...
    _daq_functions = ['stop', 'begin', 'end', 'configure', 'connect', 'disconnect', 
    ...
    def init(self, instrument=None, host=None,
...
#                'config': config,
    ...
    def __getattr__(self, attr):
        ...
        if attr in self._daq_functions:
...
def show_partition():
    ...
    """
    return Daq._pydaq['control'].partition() 

def disconnect(**kwargs):
    """Disconnect the python daq interface.
    """
    if Daq._pydaq['connected']:
        Daq._pydaq['control'].disconnect()
        Daq._pydaq.update({'connected': False})
        Daq._pydaq.update({'state': 'disconnected'})
        return 1
...
def connect(**kwargs):
    ...
    """
    if not Daq._pydaq['init']:
        daq = Daq(**kwargs)    
    
    if not get_state('connected'):
        try:
            control().connect()
            Daq._pydaq.update({'connected': True})
...
def begin(auto_config=True, new_run=False, wait=False, **kwargs):
...
def get_state(*arg):
    ...
    """
    if len(arg) > 0:
        if Daq._pydaq['state'] is arg[0]:
            return True
        else:
            return False
    else:
        return Daq._pydaq['state']

def update_state(state):
    Daq._pydaq.update({'state': state})
...
def connected():
    return Daq._pydaq['connected']

def control(**kwargs):
    """Returns pydaq control object.
    """
    return Daq._pydaq['control']
...
#    setattr(sys.modules['daq'], attr, getattr(daq._pydaq['control'], attr))
